...
export function useWorkflowViewManagement() {
   ...
   */
  async function setVueFlowInstance(internalId: string, instance: ManagedVueFlowInstance | null) { // Make async
    ...
    // if (instance) { // // <-- 移除主动状态同步
    //   // 确保实例加载时同步状态
    //   // const nodes = state.elements.filter((el): el is VueFlowNode => !('source' in el));
    //   // const edges = state.elements.filter((el): el is VueFlowEdge => 'source' in el);
    //   // instance.setNodes(nodes);
    //   // instance.setEdges(edges);
    //   // instance.setViewport(state.viewport);
    //   // console.debug(`useWorkflowViewManagement: Restored elements and viewport for tab ${internalId}`);
    // } // // <-- 移除主动状态同步
  }
   ...
   */
  async function setViewport(internalId: string, viewport: Viewport) { // Make async
    // 确保状态存在并获取它。传递 false 以避免在此处应用默认工作流。
    const state = await workflowManager.ensureTabState(internalId, false); // 添加 await
    if (!state) {
  ...
  function updateEdgeStylesForTab(internalId: string) {
    ...
    console.debug(`[useWorkflowViewManagement] 开始为标签页 ${internalId} 更新边样式 (isDark: ${isDark.value})`);
    ...
    if (instance) {
      ...
      console.debug(`[useWorkflowViewManagement] 已更新标签页 ${internalId} 的 VueFlow 实例中的边样式。`);
    }
  }
