...
/** 自动计算节点宽度时的上限，防止过宽 */
export const AUTO_CALC_MAX_WIDTH = 500
 ...
 */
export function useNodeResize(props: Readonly<UseNodeStateProps>) {
  ...
  /**
   * 计算节点根据其内容（标题、描述、输入、输出）所需的最小宽度。
   * 这是自动计算初始宽度的核心逻辑。
   * @param title - 节点标题 (label 或 displayName)
   * @param description - 节点描述 (可选)
  ...
  const calculateMinWidth = (
    ...
    const titleWidth = measureTextWidth(title, NODE_TITLE_FONT)
    // 考虑头部左右内边距 (px-2 -> 8*2=16) 和可能的 ID 徽章、图标等额外空间 (估算为 16)
    requiredWidth = Math.max(requiredWidth, titleWidth + 16 + 16)

    // --- 2. 计算描述宽度 ---
    // 节点主体区域 (custom-node-body)
    // 注意：BaseNode.vue 中描述目前未直接显示，但保留计算逻辑以备将来使用
    if (description) {
      const descWidth = measureTextWidth(description, NODE_DESC_FONT)
      // 考虑主体左右内边距 (px-1 -> 4*2=8)
      requiredWidth = Math.max(requiredWidth, descWidth + 8)
    }
    ...
    if (inputs && Object.keys(inputs).length > 0) {
      for (const key in inputs) {
        ...
        if (hasInlineInput) {
          ...
          const widthFromParamName = (paramNameWidth / (2 / 5)) + PARAM_LEFT_HANDLE_SPACE + PARAM_NAME_INLINE_INPUT_GAP
          // 2. 基于内联输入最小宽度 (假设输入占 grid 宽度的 3/5):
          const widthFromInlineInput = (INLINE_INPUT_MIN_WIDTH / (3 / 5)) + PARAM_LEFT_HANDLE_SPACE + PARAM_NAME_INLINE_INPUT_GAP
          inputLineWidth = Math.max(widthFromParamName, widthFromInlineInput) // 取两者中较大的估算值
        } else if (isMultiline) {
          // 多行布局：
          ...
          // 仅有连接点，无内联/多行/按钮输入 (如 ANY 类型)
          // 布局：| Handle | 参数名 (col-span-2 / 全宽) |
          inputLineWidth = paramNameWidth + PARAM_LEFT_HANDLE_SPACE;
        }
    ...
    if (maxSlotNameWidth > 0) {
      ...
      const slotBasedWidth = (maxSlotNameWidth * 1.1) / 0.4; // 估算基于最宽插槽名称的宽度
      // 加上主体左右内边距 (px-1 -> 4*2=8)
      const calculatedTotalWidth = slotBasedWidth + 8;
      // 更新所需宽度，确保不小于基于插槽名称的估算值
      requiredWidth = Math.max(requiredWidth, calculatedTotalWidth);
    }
    ...
    return Math.min(AUTO_CALC_MAX_WIDTH, Math.max(AUTO_CALC_MIN_WIDTH, Math.ceil(requiredWidth))) // 返回向上取整后的宽度
  }
  ...
  onMounted(() => {
    ...
    try {
      ...
      if (isGroupIONode) {
          ...
          initialNodeWidth = Math.min(AUTO_CALC_MAX_WIDTH, Math.max(MIN_NODE_WIDTH, nodeDefinition.width));
      } else if (isNodeGroupNode) { // NodeGroup 检查移到这里
          ...
          const nodeDesc = props.data.description || undefined
