...
const renderedHtml = computed(() => {
  ...
  renderer.link = (options: Tokens.Link) => {
    ...
    return `<a href="${finalHref}" target="_blank" ${
      safeTitle ? `title="${safeTitle}"` : ""
    } rel="noopener noreferrer">${text}</a>`;
  };
  marked.setOptions({ renderer, breaks: true }); // 添加 breaks: true 以支持 GFM 换行

  const rawHtml = marked.parse(props.markdownContent || "") as string; // 添加 || '' 防止 content 为 undefined
...
.markdown-content {
  pointer-events: auto; /* 允许与 Markdown 内容交互，例如点击链接 */
  ...
  /* 移除所有针对内部 p, ul, li, code, pre 等元素的样式 */
    ...
    & :deep(ol) {
      ...
      /* margin-block-start: 0.5em; */ /* 保留一些垂直间距 */
      /* margin-block-end: 0.5em; */
      /* padding-inline-start: 1.5em; */ /* 保留一级列表的缩进 */
    }
    ...
    & :deep(ol ul) {
      /* 重置默认内外边距 */
      margin-top: 0.2em; /* 稍微增加嵌套列表与其父项之间的间距 */
      margin-bottom: 0.2em;
      margin-left: 0; /* 重置 margin-left */
      padding-left: 0.8em; /* 使用 padding-left 控制缩进 */
    }
