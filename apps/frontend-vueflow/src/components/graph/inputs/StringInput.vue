<template>
  <div ref="rootRef" class="string-input relative w-full flex items-center min-w-0"> <!-- Added ref -->
    <input ref="inputRef" type="text" :value="modelValue" :placeholder="placeholder" :disabled="disabled"
      @change="handleChange" class="flex-1 px-2 py-1 text-xs rounded border transition-colors duration-200 h-6 min-w-0
                ...
                hover:border-gray-400 dark:hover:border-gray-500" :class="{
                  ...
                  'rounded-r-none': hasSuggestions // Remove right rounding if dropdown trigger is present
                }" autocomplete="off" />
    <!-- Dropdown Trigger -->
    <button v-if="hasSuggestions" ref="triggerRef" type="button" @click.stop="toggleDropdown" :disabled="disabled"
...
import { ref, computed, nextTick } from 'vue' // Added onMounted, onBeforeUnmount
import { useVueFlow } from '@vue-flow/core' // Import useVueFlow
import SuggestionDropdown from '../../common/SuggestionDropdown.vue' // Import the new component
