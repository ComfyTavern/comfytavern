<template>
  <div
    ...
    <div class="flex items-center space-x-4 flex-grow"> <!-- Added flex-grow -->
      <div>一个神秘的编辑器</div>
      <!-- 工作流菜单触发器 -->
      <div class="relative">
        <button ref="workflowButtonRef" @click="toggleWorkflowMenu" class="status-bar-button">工作流</button>
        <WorkflowMenu ref="workflowMenuRef" v-if="showWorkflowMenu" @close="showWorkflowMenu = false" />
      </div>
      <!-- 标签栏 -->
      <TabBar class="flex-grow" /> <!-- Added TabBar and flex-grow -->
    ...
    <div class="flex items-center space-x-4"> <!-- 增加间距 -->
      ...
      <button @click="togglePreview" :title="isPreviewEnabled ? '禁用实时预览' : '启用实时预览'"
        class="px-2 py-1 rounded border border-transparent text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 focus:outline-none transition-all duration-150"
        :class="{
          'text-blue-600 dark:text-blue-400 border-blue-300 dark:border-blue-700 bg-blue-50 dark:bg-blue-900/50': isPreviewEnabled,
      ...
      <button @click="handleExecuteWorkflow"
        class="px-3 py-1 rounded text-white font-medium bg-green-500 hover:bg-green-600 dark:bg-green-600 dark:hover:bg-green-700 transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-green-500">执行</button>
      <!-- 显示工作流执行状态 -->
      <span v-if="workflowStatusText" class="flex items-center space-x-1.5 text-xs font-medium ml-2" :class="{
        ...
        'text-gray-600 dark:text-gray-400': currentWorkflowStatus === ExecutionStatus.IDLE || currentWorkflowStatus === ExecutionStatus.QUEUED || currentWorkflowStatus === ExecutionStatus.SKIPPED
      }">
        <span class="inline-block h-2 w-2 rounded-full" :class="{
          ...
          'bg-gray-400': currentWorkflowStatus === ExecutionStatus.IDLE || currentWorkflowStatus === ExecutionStatus.QUEUED || currentWorkflowStatus === ExecutionStatus.SKIPPED
        }"></span>
...
import { ExecutionStatus, WebSocketMessageType, type ExecuteWorkflowPayload } from '@comfytavern/types'; // 导入消息类型和负载类型
...
// Computed property for the active tab's dirty status (Removed as it's no longer used)
// const isActiveTabDirty = computed(() => {
//   return activeTabId.value ? workflowStore.isWorkflowDirty(activeTabId.value) : false;
// });

// const currentWorkflowName = computed(() => { // Removed
//   return activeWorkflowData.value ? activeWorkflowData.value.name : '未命名工作流';
// });
...
const workflowStatusText = computed(() => {
  ...
  switch (currentWorkflowStatus.value) {
    case ExecutionStatus.IDLE: return '空闲';
    case ExecutionStatus.QUEUED: return '排队中'; // Use QUEUED
    case ExecutionStatus.RUNNING: return '运行中...';
    case ExecutionStatus.COMPLETE: return '已完成'; // Use COMPLETE
    case ExecutionStatus.ERROR: return '错误';
    case ExecutionStatus.SKIPPED: return '已跳过'; // SKIPPED is correct
    default: return '';
  }
});
...
const handleExecuteWorkflow = () => {
  ...
  if (!currentWorkflowValue || !currentWorkflowValue.nodes || currentWorkflowValue.nodes.length === 0) {
...
/* Base button style - can be removed if all buttons have specific styles now */
/* .status-bar-button {
  @apply px-3 py-1 rounded text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors;
} */

/* Removed specific styles for execution button and status indicator as they are now inline */
