...
<template>
  <BaseModal
    ...
    @close="handleSkip"
    :bare="true"
    dialogClass="bg-gradient-to-br from-gray-100 to-gray-200 text-gray-900 dark:from-slate-900 dark:to-gray-800 dark:text-white rounded-xl shadow-2xl overflow-hidden"
    contentClass="!p-0 flex flex-col"
  >
    <div class="flex-grow p-8 space-y-8 overflow-y-auto">
      <div class="text-center">
        <div class="text-center">
          <svg class="mx-auto h-16 w-16 text-blue-500 dark:text-blue-400 mb-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
            <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path>
            <circle cx="12" cy="7" r="4"></circle>
          </svg>
          <h2 class="text-3xl font-bold tracking-tight text-blue-500 dark:text-blue-400">欢迎使用 ComfyTavern</h2>
          <p class="mt-3 text-sm text-gray-500 dark:text-gray-400 max-w-xs mx-auto p-3">
            请设置您的初始信息
          </p>
        </div>
        ...
        <form @submit.prevent="handleSave" class="space-y-6">
          <div class="relative">
            ...
            <input
              ...
              :class="[
                'w-full !pl-12 !pr-4 !py-3 !bg-gray-100 dark:!bg-gray-700/50 !text-gray-900 dark:!text-white placeholder-gray-400 dark:placeholder-gray-500 !rounded-lg !shadow-sm transition-all',
                errorMessage
                  ? '!border-red-500 dark:!border-red-400 focus:!ring-red-500 focus:!border-red-500'
                  : '!border-gray-300 dark:!border-gray-600 focus:!ring-blue-500 focus:!border-blue-500'
              ]"
            />
            <p v-if="errorMessage" class="mt-2 text-xs text-red-500 dark:text-red-400 flex items-center justify-start text-left">
          ...
          <div class="pt-4 space-y-3 sm:space-y-0 sm:flex sm:flex-row-reverse sm:space-x-reverse sm:space-x-3">
            ...
            <button
              type="button"
              @click="handleSkip"
              class="w-full sm:w-auto px-8 py-3 text-base font-medium text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-gray-700/60 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 focus:ring-offset-white dark:focus:ring-offset-gray-800 rounded-lg disabled:opacity-50 transition-colors duration-150 ease-in-out"
            >
              稍后决定
            </button>
          </div>
    ...
    <div class="px-8 py-3 text-xs text-center text-gray-400 dark:text-gray-500 shrink-0">
      您可以稍后在设置中更改昵称。
    </div>
  </BaseModal>
</template>
