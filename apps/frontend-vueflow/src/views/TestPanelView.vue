<template>
  <div class="p-6 max-w-5xl mx-auto">
    <InitialUsernameSetupModal
      ...
      @close="isInitialUsernameSetupModalVisible = false; uiStoreResult = t('testPanel.uiStore.resultsContent.initialUsernameClosed')"
      @saved="isInitialUsernameSetupModalVisible = false; uiStoreResult = t('testPanel.uiStore.resultsContent.initialUsernameSaved')"
    />
    <h1 class="text-3xl font-bold mb-8 text-text-base">{{ t('testPanel.title') }}</h1>

    <!-- DialogService 测试 -->
    <section class="mb-12">
      <h2 class="text-2xl font-semibold mb-6 text-text-base border-b pb-2">{{ t('testPanel.dialogService.title') }}
      </h2>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
        <!-- 对话框部分 -->
        <div class="bg-background-surface p-6 rounded-lg shadow-md">
          <h3 class="text-xl font-semibold mb-4 text-text-secondary">{{ t('testPanel.dialogService.dialogs') }}</h3>
          <div class="space-y-4">
            <button @click="showMessageDialog" class="btn btn-blue w-full">{{ t('testPanel.dialogService.buttons.showMessage') }}</button>
            <button @click="showConfirmDialog" class="btn btn-green w-full">{{ t('testPanel.dialogService.buttons.showConfirm') }}</button>
            <button @click="showDangerConfirmDialog" class="btn btn-red w-full">{{ t('testPanel.dialogService.buttons.showDangerConfirm') }}</button>
            <button @click="showAutoCloseDialog" class="btn btn-purple w-full">{{ t('testPanel.dialogService.buttons.showAutoClose') }}</button>
            <button @click="showInputDialog" class="btn btn-teal w-full">{{ t('testPanel.dialogService.buttons.showInput') }}</button>
            <button @click="showTextareaDialog" class="btn btn-cyan w-full">{{ t('testPanel.dialogService.buttons.showTextarea') }}</button>
            <button @click="showLargeTextMessageDialog" class="btn btn-orange w-full">{{ t('testPanel.dialogService.buttons.showLargeMessage') }}</button>
          </div>
        </div>
      ...
      <div class="mt-8 bg-background-surface p-6 rounded-lg shadow-md">
        <h3 class="text-lg font-semibold mb-3 text-text-secondary">{{ t('testPanel.dialogService.results') }}</h3>
    ...
    <!-- UiStore Modals 测试 -->
    <section>
      <h2 class="text-2xl font-semibold mb-6 text-text-base border-b pb-2">{{ t('testPanel.uiStore.title') }}</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
        <div class="bg-background-surface p-6 rounded-lg shadow-md">
          <h3 class="text-xl font-semibold mb-4 text-text-secondary">{{ t('testPanel.uiStore.specificModals') }}</h3>
          <div class="space-y-4">
            <button @click="openSettings" class="btn btn-indigo w-full">{{ t('testPanel.uiStore.buttons.openSettings') }}</button>
            <button @click="openRegexEditor" class="btn btn-pink w-full">{{ t('testPanel.uiStore.buttons.openRegexEditor') }}</button>
            <button @click="openInitialUsernameSetup" class="btn btn-sky w-full">{{ t('testPanel.uiStore.buttons.openInitialUsernameSetup') }}</button>
          </div>
        </div>
      </div>
      <div class="mt-8 bg-background-surface p-6 rounded-lg shadow-md">
        <h3 class="text-lg font-semibold mb-3 text-text-secondary">{{ t('testPanel.uiStore.results') }}</h3>
        <div class="bg-background-base p-4 rounded-md min-h-[50px]">
    ...
    <section class="mt-12">
      <h2 class="text-2xl font-semibold mb-6 text-text-base border-b pb-2">{{ t('testPanel.settingsControl.title') }}</h2>
      <div class="bg-background-surface p-6 rounded-lg shadow-md">
        <SettingsPanel :config="testSettingItems" />
      </div>
      <!-- 用于验证 settingsStore 中的值是否正确更新 -->
      <div class="mt-8 bg-background-surface p-6 rounded-lg shadow-md">
        <h3 class="text-lg font-semibold mb-3 text-text-secondary">{{ t('testPanel.settingsControl.results') }}</h3>
...
async function executeDialogAction<T>(
  ...
  dialogServiceResult.value = t('testPanel.dialogService.resultsContent.waitingFor', { actionName });
  try {
    const result = await action();
    ...
    if (error === 'cancelled' || error === null || String(error).toLowerCase().includes('cancel')) {
      ...
      dialogServiceResult.value = t('testPanel.dialogService.resultsContent.exception', { actionName, error });
    }
  }
}
...
const showConfirmDialog = async () => {
  await executeDialogAction(
    ...
    (confirmed) => t('testPanel.dialogService.resultsContent.confirmSuccess', { choice: confirmed ? t('common.confirm') : t('common.cancel') }),
...
const showDangerConfirmDialog = async () => {
  await executeDialogAction(
    ...
    (confirmed) => t('testPanel.dialogService.resultsContent.dangerConfirmSuccess', { choice: confirmed ? t('common.delete') : t('common.cancel') }),
...
const showInfoToast = () => {
  dialogService.showInfo(t('testPanel.dialogService.dialogContent.infoToastText'), t('testPanel.dialogService.dialogContent.infoToastTitle'));
  dialogServiceResult.value = t('testPanel.dialogService.resultsContent.infoToastTriggered');
};
const showSuccessToast = () => {
  dialogService.showSuccess(t('testPanel.dialogService.dialogContent.successToastText'), t('testPanel.dialogService.dialogContent.successToastTitle'));
  dialogServiceResult.value = t('testPanel.dialogService.resultsContent.successToastTriggered');
};
const showWarningToast = () => {
  dialogService.showWarning(t('testPanel.dialogService.dialogContent.warningToastText'), t('testPanel.dialogService.dialogContent.warningToastTitle'));
  dialogServiceResult.value = t('testPanel.dialogService.resultsContent.warningToastTriggered');
};
const showErrorToast = () => {
  dialogService.showError(t('testPanel.dialogService.dialogContent.errorToastText'), t('testPanel.dialogService.dialogContent.errorToastTitle'));
...
const showMultipleToasts = () => {
  ...
  setTimeout(() => dialogService.showSuccess(t('testPanel.dialogService.dialogContent.multiToast2')), 500);
  setTimeout(() => dialogService.showWarning(t('testPanel.dialogService.dialogContent.multiToast3')), 1000);
...
const openRegexEditor = () => {
  ...
  uiStore.openRegexEditorModal({
    ...
    onSave: (updatedRules) => {
      ...
      uiStoreResult.value = t('testPanel.uiStore.resultsContent.regexEditorSaved', { count: updatedRules.length });
