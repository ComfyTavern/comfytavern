...
export class MathOperationNodeImpl {
  ...
  static async execute(inputs: Record<string, any>, context: any): Promise<Record<string, any>> {
    ...
    const executionMap: Record<string, (inputs: Record<string, any>) => Promise<Record<string, any>>> = {
      ...
      'multiply': this.executeMultiply,
      'divide': this.executeDivide,
      'power': this.executePower,
      'sqrt': this.executeSqrt,
      'inverse_sqrt': this.executeInverseSqrt,
      'logarithm': this.executeLogarithm,
      'absolute': this.executeAbsolute,
      'exp': this.executeExp,
      'min': this.executeMin,
      'max': this.executeMax,
      'lessThan': this.executeLessThan,
      'greaterThan': this.executeGreaterThan,
      'sign': this.executeSign,
      'round': this.executeRound,
      'floor': this.executeFloor,
      'ceil': this.executeCeil,
      'truncate': this.executeTruncate,
      'fraction': this.executeFraction,
      'modulo': this.executeModulo,
      'sin': this.executeSine,
      'cos': this.executeCosine,
      'tan': this.executeTangent,
      'asin': this.executeAsin,
      'acos': this.executeAcos,
      'atan': this.executeAtan,
      'atan2': this.executeAtan2,
      'sinh': this.executeSinh,
      'cosh': this.executeCosh,
      'tanh': this.executeTanh,
      'to_radians': this.executeToRadians,
      'to_degrees': this.executeToDegrees,
      'multiply_add': this.executeMultiplyAdd,
    };

    const executor = executionMap[currentModeId] || this.executeAdd;
