...
项目推荐使用 **Prettier** 进行代码格式化。由于项目层面未强制统一的 Prettier 配置文件，开发者应依赖各自 IDE（如 VSCode）的 Prettier 插件，并配置为**保存时自动格式化 (Format On Save)**。这有助于保持个人代码风格的一致性，并确保提交到代码库的代码符合通用可读性标准。

## 3. 代码风格与质量检查 (TypeScript & WebHint)

项目主要依赖 **TypeScript 类型检查**结合 **WebHint** (`.hintrc`) 进行代码质量和潜在错误检测，以提高代码可靠性和可维护性。

- **WebHint 配置**: WebHint 的配置文件位于项目根目录下的 [`.hintrc`](.hintrc:1)。其配置继承了 `"development"`，并关闭了 `"typescript-config/consistent-casing"` 提示。
- **IDE 集成**: 建议开发者在 IDE 中安装 WebHint 插件（如果可用），以便在编码过程中实时获得检查警告。代码的实时类型检查由 IDE 的 TypeScript 语言服务提供。
- **代码检查命令**:
  项目通过以下命令强调代码的类型检查：
  - `bunx vue-tsc --build apps/frontend-vueflow/tsconfig.json`：用于前端 Vue/TypeScript 代码的类型检查。
  - `bun tsc -p apps/backend/tsconfig.json --noEmit`：用于后端 TypeScript 代码的类型检查。
  开发者应确保其代码通过了这些类型检查。
