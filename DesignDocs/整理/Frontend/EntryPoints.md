...
3.  **注册核心插件**：
    ...
    *   **VueFlow**：虽然没有直接在 `main.ts` 中看到 `app.use(VueFlow)`，但 VueFlow 的核心功能通常是通过在组件中导入和使用其特定组件（如 `<VueFlow />`）来实现的。其样式已在步骤1中导入。
...
-   **集成全局 UI 组件**：包含一些需要在整个应用层面显示的组件，如全局对话框容器、Tooltip 渲染器、设置模态框等。
...
-   **状态初始化与响应式引用**：
    ...
    *   使用 `storeToRefs` 从 store 中获取响应式状态，如 `isDark`, `activeTabId`, `currentProjectId`, `isSettingsModalVisible`, `userContext`, `currentUser` 等 ([`apps/frontend-vueflow/src/App.vue:26-30`](apps/frontend-vueflow/src/App.vue:26-30))。
