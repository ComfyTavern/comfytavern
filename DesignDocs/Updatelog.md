...
- feat(插件与UI): 增强插件系统与用户界面交互
  - **插件配置UI**: 引入了完整的插件配置用户界面，允许用户在模态框中直接查看和编辑插件设置。
  - **流式连接视觉样式**: 为流式数据连接（`isStream: true`）增加了独特的动画虚线视觉效果，增强了工作流的可读性。
  - **组接口流式配置**: 允许在节点组的接口编辑器中为每个输入/输出单独设置“是否为流”的选项，并修复了相关属性丢失的bug。
- refactor(UI与类型): 统一模态框管理与重构类型定义
  - **集中式模态框服务**: 重构了整个应用的模态框系统，采用集中式的动态组件服务进行管理，显著简化了API并减少了样板代码。
  - **统一模态框插槽**: `BaseModal` 组件统一使用 `#content` 具名插槽注入内容，提高了API的一致性。
  - **类型定义迁移**: 将设置相关的类型定义迁移至共享的 `@comfytavern/types` 包，以提高代码复用性。
- fix(流处理): 修复流处理与WebSocket背压的关键错误
  - 解决了`ExecutionEngine`中流被提前丢弃、`GroupOutputNode`阻塞上游以及WebSocket背压处理不当导致消息顺序错乱等多个严重问题。
- feat(i18n与WebSocket): 国际化增强与通信优化
  - **i18n**: 国际化了历史记录面板；新增了 `i18n:copy` 脚本以简化语言包更新流程。
  - **WebSocket**: 实现了场景订阅模式，优化了消息队列和背压处理机制，提升了实时通信的健壮性。
- docs: 完善文档
  - 新增了前端扩展点分析与实现方案文档。
  - 更新了对话服务、i18n开发流程等相关文档。

## 2025 年 7 月 26 日

- refactor(状态管理): 完成`workflowStore`重构，实现模块化
  - 通过将逻辑迁移到独立的、按领域划分的action模块（如`nodeActions`, `edgeActions`），彻底移除了庞大的`useWorkflowInteractionCoordinator`，完成了“内阁制”架构的实施，大幅提升了代码的可维护性和模块化程度。
- feat(文档站点): 初始化并部署VitePress文档站点
  - 在 `docs-site/` 目录下创建了全新的文档站点，支持中英双语。
  - 配置了多语言路由、导航栏、侧边栏，并添加了自动部署到GitHub Pages的CI/CD工作流。
- fix(核心功能): 修复LLM配置、历史记录与UI显示问题
  - **LLM**: 修复了OpenAI流式适配器丢失`usage`数据的问题；防止了在切换提供商时覆盖用户自定义的Base URL。
  - **历史记录**: 增强了历史详情的格式化逻辑，修复了特殊字符的显示问题。
  - **UI**: 修复了标签页标题过长溢出的问题，并优化了滚动性能。
- docs: 更新文档
  - 更新了多个README文件，统一链接到新的在线文档站点。
  - 重写了历史记录管理指南以匹配最新的架构。

## 2025 年 7 月 25 日

- refactor(状态管理): 启动`workflowStore`重构计划
  - 为解决`workflowStore`与`useWorkflowInteractionCoordinator`高度耦合的问题，引入了新的“内阁制”架构设计方案。
  - 搭建了新的目录结构和类型接口，为后续从`coordinator`中渐进式迁移逻辑奠定了基础。

## 2025 年 7 月 23 日

- docs: 完善项目文档与展示
  - 在所有 README 文件中添加了 Ask DeepWiki 和 ComfyTavern Beta 版的徽章，提升项目识别度。
  - 全面更新了多语言 README 文件和项目设计文档，以反映最新的架构、功能变更（如插件系统、文件资产管理）和统一的目录结构。

## 2025 年 7 月 22 日

- docs: 优化核心架构文档
  - 重构并更新了 Agent 架构、工具系统统一架构 (`TAM`)、文件与资产管理指南等核心设计文档，去除了冗余信息，澄清了核心概念，并使其与最新实现保持一致。

## 2025 年 7 月 21 日

- feat(项目与数据列表): 增强项目管理与列表交互
  - 实现了项目的批量删除功能，包括前端UI、后端API、确认对话框、多选操作和国际化支持。
  - 为数据列表组件增加了全选和清除选择功能，并优化了项目列表视图的交互。
- docs: 文档与脚本优化
  - 优化了“智能更新日志”工作流的文档，详细说明了分组和提炼提交记录的步骤，并强调了非覆盖式写入的重要性。
